...
import com.icicisecurities.hub.ui.fatca.model.GetAddressProofListener;
import com.icicisecurities.hub.utils.AppConstants;

import java.util.ArrayList;
import java.util.regex.Pattern;

public class AddressProofTask implements NetworkResponseListener {

        //Medium priority NON-UI variables goes below.....
        private Context context;
        private Object instanceOfClass;
        private GetAddressProofListener getAddressProofListener;

        //Least priority variables goes below....
        private final String TAG = "CountryTask";

        public AddressProofTask(Context context, Object instanceOfClass) {
        ...
        public void getDetails(){
            ...
            PostResponseAsync getSpinnersData = new PostResponseAsync(context, AddressProofTask.this);
            getSpinnersData.setResponseListener(AddressProofTask.this);
        ...
        public void notifyNetworkResponseSuccess(String response) {
            if(response != null) {
                if (response.startsWith(AppConstants.SUCCESS_STRING_PREFIX)) {
                    ...
                    FatcaVO fatcaVO;
                    fatcaVO = new FatcaVO();
                    fatcaVO.setCountryName("Select AddressProof");
                    countryListAl.add(fatcaVO);

                    for (int i = 0; i < countryObject.length; i++) {

                        fatcaVO = new FatcaVO();
//                        countryList.split(Pattern.quote(AppConstants.TILDE_SYMBOL))[0].split(Pattern.quote("^"))
                        String[] countryDetails = countryObject[i].split(Pattern.quote(AppConstants.CARET_SYMBOL));
                        fatcaVO.setCountryName(countryDetails[1]);
                        fatcaVO.setCountryCode(countryDetails[0]);
                        countryListAl.add(fatcaVO);
                    }//for (int i = 0; i < ocuupationsList.length; i++) closes here.....
                    getAddressProofListener.getAddressProofList(countryListAl);
                }
            }
        }
        ...
        public void setDetailsListener(GetAddressProofListener getStateListenr) {
            // TODO Auto-generated method stub
            this.getAddressProofListener = getStateListenr;
