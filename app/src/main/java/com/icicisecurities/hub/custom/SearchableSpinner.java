...
import com.icicisecurities.hub.ui.summary.FilterAdapter;
...
public class SearchableSpinner extends AppCompatSpinner implements View.OnTouchListener,
    ...
    private void init() {
        ...
        if (!TextUtils.isEmpty(_strHintText)) {
            ArrayAdapter arrayAdapter = new ArrayAdapter(_context, R.layout.single_row_leads_filter_drop_down, new String[]{_strHintText});
            _isFromInit = true;
        ...
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            if (null != _arrayAdapter) {
                ...
                for (int i = 0; i < _arrayAdapter.getCount(); i++) {
                    _items.add(((FilterAdapter) _arrayAdapter).getItem(i).getEntityDescription());
                }
                // Change end.
        ...
        if (!_isFromInit) {
            ...
            if (!TextUtils.isEmpty(_strHintText) && !_isDirty) {
                ArrayAdapter arrayAdapter = new ArrayAdapter(_context, R.layout
                        .spinner_textview, new String[]{_strHintText});
